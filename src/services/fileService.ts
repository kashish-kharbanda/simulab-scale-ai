...
class FileService {
  ...
  validateFile(file: File): { valid: boolean; reason?: string } {
    if (file.size > MAX_FILE_SIZE) {
      return {
        valid: false,
        reason: `File is too large (${(file.size / 1024 / 1024).toFixed(2)}MB). Maximum size is ${MAX_FILE_SIZE / 1024 / 1024}MB.`
      };
    }
   ...
   */
  async uploadFile(file: File, purpose: string = 'capybara-attachment'): Promise<FileAttachment | null> {
    try {
   ...
   */
  async uploadFiles(files: File[]): Promise<{ attachments: FileAttachment[], failedCount: number }> {
    const formData = new FormData();
