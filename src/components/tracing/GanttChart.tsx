...
export function GanttChart({
  ...
  const processedSpans = useMemo(() => {
    ...
    const sortedRootSpans = [...rootSpans].sort((a, b) => {
      const aTime =
        a.start_time instanceof Date ? a.start_time.getTime() : new Date(a.start_time).getTime();
      const bTime =
        b.start_time instanceof Date ? b.start_time.getTime() : new Date(b.start_time).getTime();
      return aTime - bTime;
    });
  ...
  function inferTypeFromName(name: string): string {
    ...
    if (name.includes('reason') || name.includes('understand') || name.includes('planning'))
      return 'reasoning';
  ...
  const scrollToSpan = (span: Span) => {
    ...
    const startTime = span.start_time instanceof Date ? span.start_time : new Date(span.start_time);
  ...
  const animateScroll = (
    ...
    const step = (currentTime: number) => {
      ...
      if (options.left !== undefined) {
        element.scrollLeft = startLeft + (targetLeft - startLeft) * easeOutQuint(progress);
      }
  ...
  const scrollToBottom = () => {
    ...
    const maxHorizontalScroll = timelineContainer.scrollWidth - timelineContainer.clientWidth;
    ...
    if (parentContainer) {
      ...
      setTimeout(() => {
        const maxVerticalScroll = parentContainer.scrollHeight - parentContainer.clientHeight;
    ...
    const mainContainer = document.querySelector('main') || document.querySelector('.h-screen');
    ...
    <TooltipProvider>
      <div className="relative h-full flex flex-col">
        ...
        <div className="flex mb-2 flex-shrink-0">
          ...
          <div className="flex-1 relative px-4 pr-8 overflow-hidden">
            ...
            <div className="absolute bottom-0 left-4 right-8 h-[12px]">
              {duration > 100 &&
                Array.from({ length: 5 }).map((_, i) => {
                  ...
                  return (
                    <div key={`marker-${i}`} className="absolute" style={{ left: markerPosition }}>
                      <div className="h-1 w-[1px] bg-gray-300" />
                      <span className="text-xs text-muted-foreground mt-1 transform -translate-x-1/2">
        ...
        <div className="flex-1 border rounded-md overflow-hidden">
          ...
          <div className="h-full overflow-y-auto">
            ...
            <div className="flex min-h-full">
              ...
              <div className="w-[250px] bg-background relative">
                {/* Shadow effect */}
                <div className="absolute inset-y-0 right-0 w-[10px] z-10 pointer-events-none shadow-[4px_0_10px_-3px_rgba(0,0,0,0.1)]"></div>
                {/* Span names content */}
                  ...
                  {processedSpans.flattenedSpans.map((span, index) => {
                    ...
                    return (
                      <div
                        key={`label-${span.id}`}
                        className={cn(
                          'relative flex items-center mb-3 rounded group',
                          isSelected
                      ...
                      >
                        <div
                          className="flex items-center text-sm font-medium truncate w-full h-full cursor-pointer"
              ...
              <div className="flex-1 overflow-x-auto" ref={timelineContainerRef}>
                <div
                  className="p-4 pr-8 chart-container relative min-h-full"
                  ...
                  {processedSpans.flattenedSpans.map((span, index) => {
                    ...
                    return (
                      <div
                        key={`bar-${span.id}`}
                        className={cn(
                          'relative flex items-center mb-3 rounded',
                          isSelected
                            ? 'bg-blue-100 text-blue-900 dark:bg-blue-800 dark:text-blue-100'
                            : 'hover:bg-gray-50 hover:text-black dark:hover:bg-gray-50 dark:hover:text-black',
                        )}
                      ...
                      >
                        <div className="flex-1 h-full flex items-center">
                          <div
                            className={cn(
                              'rounded cursor-pointer hover:opacity-80 transition-opacity flex items-center justify-center px-1 text-white text-xs',
                              getSpanColor(span),
                            )}
                            style={{
                              position: 'absolute',
